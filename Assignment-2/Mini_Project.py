...
try:
    ...
    artist = input("Enter Artist Name: ").title()      # first letter of every word should be capital
    artist=artist.replace(' ','+')  # url has plus sign instead of space

    resp = requests.get(
        f"http://ws.audioscrobbler.com/2.0/?method=artist.gettoptracks&artist={artist}&api_key={API_KEY}&format=json"
        )
    toptrack = resp.json()
                            # extracting data from json not easy ;-;
    song_list = []
    for song in toptrack["toptracks"]["track"]:     # list of top songs
        song_list.append(song["name"])
    for i in range(len(song_list)):
        song_list[i] = song_list[i].replace(" ", "+")       # url has plus sign instead of space
    ...
    for song in song_list:      # extracting song data
        ...
        try:
            ...
            print(str(i)+')',name+',',artist.replace('+',' ')+',',release_date)
            i+=1
            print()
            print('Song Trivia:')
            print('------------')
